	...
	function forkWorker() {
		...
		worker
			...
			.on('exit', function (code, signal) {
				...
				try {
					shell.exec('mkdir -p ~/crashlogs_admin');
					var crashFileName = '~/crashlogs_admin/' + new Date().toISOString().replace(/[:\.-]/g, '_') + '.log';
					shell.exec('tail -150 ~/node_admin.out |tee ' + crashFileName);
					...
					var transporter = nodemailer.createTransport({
						...
						auth: {
							...
							pass: 'eef8656115dcdc75f614d20aa4cc0a20'
						}
					});
					transporter.sendMail({
						from: '"Server Crash Report" <crash@node.community>', // sender address
						to: ['kushal@storymaker.io', 'sonu@storymaker.io', 'meghna@storymaker.io','amanpreet@codalien.com'], // list of receivers
...
var localAppCluster = new LocalAppCluster(NUMBER_OF_WORKERS_PER_CPU, './keystone.js', "ActivePathways" + process.env.NODE_ENV);
//Launch sub cluster
